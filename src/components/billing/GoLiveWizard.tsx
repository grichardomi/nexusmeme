...
export function GoLiveWizard({ onClose, onComplete }: GoLiveWizardProps) {
    ...
    return (
      <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-md w-full p-8 text-center">
          ...
          <p className="mt-4 text-slate-600 dark:text-slate-400">Loading bot info...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-md w-full border border-slate-200 dark:border-slate-700 overflow-hidden">

        {/* Step: Confirm */}
        {step === 'confirm' && bots.length > 0 && (
          <>
            <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-5">
              <h2 className="text-xl font-bold">Switch to Live Trading</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {bots.length > 1 && (
                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                  ...
                  <ul className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
                    {bots.map((bot) => (
                      <li key={bot.id}>• {bot.name} {bot.isActive && '(running)'}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              <div className="text-sm text-slate-600 dark:text-slate-400 space-y-2">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <p>{bots.length === 1 ? 'Bot' : 'All bots'} will be stopped briefly if running</p>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-green-500 mt-0.5">2.</span>
                  <p>Trading mode switches from paper to live for {bots.length === 1 ? 'the bot' : 'all bots'}</p>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-green-500 mt-0.5">3.</span>
                  <p>{bots.length === 1 ? 'Bot restarts' : 'Bots restart'} automatically in live mode</p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4 space-y-3">
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input
                    ...
                    className="mt-0.5 w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-green-600 focus:ring-green-500 cursor-pointer"
                  />
            ...
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex gap-3">
              <button
                onClick={onClose}
                className="flex-1 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
              >
                Cancel
              </button>
              <button
                ...
                className={`flex-1 px-4 py-2.5 text-sm font-bold rounded-lg transition ${
                  feeAcknowledged
          ...
          <>
                ...
                <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4 mt-4">
                  ...
                  <ul className="text-xs space-y-1">
                    {switchResults.map((result) => (
                      <li key={result.botId} className={result.success ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
            ...
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700">
              <button
                ...
                className="w-full px-4 py-2.5 text-sm font-bold text-white bg-green-600 hover:bg-green-700 rounded-lg transition"
              >
                Done
              </button>
            </div>
        ...
        {step === 'error' && (
          <>
            ...
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700">
              <button
                onClick={onClose}
                className="w-full px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
              >
                Close
              </button>
            </div>
