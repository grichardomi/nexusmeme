      ...
      for (const trade of openTrades) {
        ...
        if (peakProfitPct > 0 && currentProfitPct < peakProfitPct) {
          ...
          if (erosionUsedFraction > erosionCapFraction) {
            const severity = this.getErosionSeverity(erosionUsedFraction, erosionCapFraction);
            const alert: PositionAlert = {
              type: 'EROSION_ALERT',
              tradeId: trade.id,
              pair: trade.pair,
              severity,
              message: `Profit erosion detected: peaked at +${peakProfitPct.toFixed(2)}%, now ${currentProfitPct.toFixed(2)}% (used ${(erosionUsedFraction * 100).toFixed(1)}% of peak vs cap ${(erosionCapFraction * 100).toFixed(1)}%)`,
              currentProfitPct,
              peakProfitPct,
